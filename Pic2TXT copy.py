...
class MY_GUI():
    ...
    def set_init_window(self):
        self.init_window_name.title("图片提取文字助手")
        
        w,h=self.init_window_name.maxsize()
        self.init_window_name.geometry("{}x{}".format(w, h))
        self.init_window_name.resizable(width=False, height=False)
        #self.init_window_name.attributes("-fullscreen",True)
        # w= self.init_window_name.winfo_screenwidth()
        # h =self.init_window_name.winfo_screenheight()-48
        

        # # self.init_window_name.resizable(width=False, height=False)
        #set_window_ceter(self,300,300)
        ...
        self.label1=Label(self.init_window_name,image=photo)
